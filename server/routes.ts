...
// Helper: geo-locate IP (best-effort, never throws)
async function geoLocate(ip: string): Promise<string> {
  try {
    if (ip === 'unknown' || ip === '::1' || ip.startsWith('127.') || ip.startsWith('10.') || ip.startsWith('192.168.')) {
      return 'Local / Internal';
    }
    const resp = await fetch(`http://ip-api.com/json/${ip}?fields=status,city,country`);
...
async function notify(memberId: string | undefined, groupId: string | undefined, type: string, title: string, message: string) {
  try {
    await storage.createNotification({ memberId, groupId, type, title, message });
  } catch (e) {
    console.error('notify error:', e);
  }
}

// Helper: create audit log (fire-and-forget)
async function audit(req: any, action: string, opts: { entityType?: string; entityId?: string; performedByName?: string; groupId?: string; details?: any } = {}) {
  try {
    const ip = getClientIp(req);
    const location = await geoLocate(ip);
    const userRole = req.userRole;
    const performedBy = req.userId || req.session?.memberSession?.member?.id;
    const performedByType = userRole ? 'staff' : 'member';
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.delete('/api/users/:id', isAuthenticated, requireRole(['admin']), async (req: any, res) => {
    try {
      ...
      audit(req, 'USER_DELETED', {
        entityType: 'user', entityId: id,
        performedByName: req.userName,
        details: { deletedUserName: `${deletedUser.firstName} ${deletedUser.lastName}`, role: deletedUser.role }
      });
  ...
  app.post('/api/auth/member-login', async (req, res) => {
    try {
      ...
      // Get active loan for due date
      const memberLoans = await storage.getLoans(member.groupId, member.id);
      const activeLoan = memberLoans.find(l => l.status === 'approved' || l.status === 'active');
      
      // Audit the member login
      audit(req, 'MEMBER_LOGIN', {
        entityType: 'member',
        entityId: member.id,
        performedBy: member.id,
        performedByName: `${member.firstName} ${member.lastName}`,
        groupId: member.groupId,
        details: { groupRole: member.groupRole, groupName: group?.name }
      });
  ...
  app.post('/api/auth/staff-login', async (req, res) => {
    try {
      ...
      // Audit the staff login
      audit(req, 'STAFF_LOGIN', {
        entityType: 'user',
        entityId: user.id,
        performedBy: user.id,
        performedByName: `${user.firstName} ${user.lastName}`,
        details: { role: user.role, userId: user.userId, location: user.location }
      });
  ...
  app.post('/api/transactions', isAuthenticated, async (req: any, res) => {
    try {
      ...
      // Notifications and audit log for savings/welfare transactions
      const memberId = transactionData.memberId;
      const groupId = transactionData.groupId;
      const txType = transactionData.type;
      const amount = parseFloat(transactionData.amount).toFixed(2);
      if (memberId) {
        const member = await storage.getMember(memberId);
        const memberName = member ? `${member.firstName} ${member.lastName}` : 'Member';
        const typeLabel = txType === 'savings' ? 'Savings' : txType === 'welfare' ? 'Welfare' : txType === 'fine' ? 'Fine' : 'Transaction';
        notify(memberId, groupId, txType,
          `${typeLabel} Recorded`,
          `UGX ${amount} ${typeLabel.toLowerCase()} recorded for ${memberName}.`
        );
        audit(req, `${txType.toUpperCase()}_CREATED`, {
          entityType: 'transaction', entityId: transaction.id,
          performedByName: req.userName,
          groupId,
          details: { memberId, memberName, type: txType, amount }
        });
      }
  ...
  app.post('/api/loans', isAuthenticated, async (req: any, res) => {
    try {
      ...
      // Notify member and audit
      if (member) {
        const memberName = `${member.firstName} ${member.lastName}`;
        notify(loan.memberId, loanData.groupId, 'loan',
          'Loan Approved & Disbursed',
          `UGX ${parseFloat(loanData.amount).toFixed(2)} loan approved for ${memberName}. Total repayable: UGX ${totalRepayment.toFixed(2)} over ${months} month(s).`
        );
        audit(req, 'LOAN_CREATED', {
          entityType: 'loan', entityId: loan.id,
          performedByName: req.userName,
          groupId: loanData.groupId,
          details: { memberId: loan.memberId, memberName, amount: loanData.amount, totalRepayment, months, purpose: loanData.purpose }
        });
      }
  ...
  app.post('/api/transactions/loan-payment', isAuthenticated, async (req: any, res) => {
    try {
      ...
      // Notify member and audit
      notify(memberId, groupId, 'loan_payment',
        'Loan Payment Received',
        `UGX ${paymentAmount.toFixed(2)} loan payment received for ${member.firstName} ${member.lastName}. Remaining balance: UGX ${newLoanBalance.toFixed(2)}.`
      );
      audit(req, 'LOAN_PAYMENT_MADE', {
        entityType: 'loan_payment', entityId: memberId,
        performedByName: processedBy,
        groupId,
        details: { memberId, memberName: `${member.firstName} ${member.lastName}`, paymentAmount, newLoanBalance }
      });
  ...
  app.get('/api/notifications', isAuthenticated, async (req: any, res) => {
    try {
      const memberSession = req.session?.memberSession;
      if (!memberSession?.member?.id) {
        return res.json([]);
      }
      const notifs = await storage.getNotifications(memberSession.member.id);
      res.json(notifs);
    } catch (error) {
  ...
  app.get('/api/notifications/unread-count', isAuthenticated, async (req: any, res) => {
    try {
      const memberSession = req.session?.memberSession;
      if (!memberSession?.member?.id) return res.json({ count: 0 });
      const cnt = await storage.getUnreadCount(memberSession.member.id);
      res.json({ count: cnt });
    } catch (error) {
      res.json({ count: 0 });
    }
  ...
  app.patch('/api/notifications/read-all', isAuthenticated, async (req: any, res) => {
    try {
      const memberSession = req.session?.memberSession;
      if (!memberSession?.member?.id) return res.json({ success: true });
      await storage.markAllNotificationsRead(memberSession.member.id);
      res.json({ success: true });
    } catch (error) {
  ...
  app.get('/api/audit-logs', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const logs = await storage.getAuditLogs({ groupId, limit });
      res.json(logs);
    } catch (error) {
