...
export interface IStorage {
  ...
  createNotification(data: { memberId?: string; groupId?: string; type: string; title: string; message: string }): Promise<Notification>;
  getNotifications(memberId: string): Promise<Notification[]>;
  getGroupNotifications(groupId: string): Promise<Notification[]>;
  markNotificationRead(id: string): Promise<void>;
  markAllNotificationsRead(memberId: string): Promise<void>;
  getUnreadCount(memberId: string): Promise<number>;

  // Audit log operations
  createAuditLog(data: { action: string; entityType?: string; entityId?: string; performedBy?: string; performedByType?: string; performedByName?: string; groupId?: string; details?: any; ipAddress?: string; userAgent?: string; location?: string }): Promise<AuditLog>;
  getAuditLogs(filters?: { groupId?: string; limit?: number }): Promise<AuditLog[]>;
...
export class DatabaseStorage implements IStorage {
  ...
  async createNotification(data: { memberId?: string; groupId?: string; type: string; title: string; message: string }): Promise<Notification> {
  ...
  async getGroupNotifications(groupId: string): Promise<Notification[]> {
    return await db.select().from(notifications)
  ...
  async createAuditLog(data: { action: string; entityType?: string; entityId?: string; performedBy?: string; performedByType?: string; performedByName?: string; groupId?: string; details?: any; ipAddress?: string; userAgent?: string; location?: string }): Promise<AuditLog> {
    const [log] = await db.insert(auditLogs).values(data).returning();
    return log;
  }

  async getAuditLogs(filters?: { groupId?: string; limit?: number }): Promise<AuditLog[]> {
    const conditions = [];
    if (filters?.groupId) conditions.push(eq(auditLogs.groupId, filters.groupId));
    const limit = filters?.limit || 200;
    if (conditions.length > 0) {
      return await db.select().from(auditLogs).where(and(...conditions)).orderBy(desc(auditLogs.createdAt)).limit(limit);
    }
