...
export default function MemberDashboard() {
  ...
  const dashboardContent = (
    <div className="space-y-6">
      ...
      {/* Group Transparency Dashboard - shown to ALL members */}
        ...
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-lg sm:text-xl">
            ...
            <span className="text-green-700 dark:text-green-400 font-bold">
              {group?.name || member.groupName || "Your Group"} — Transparency View
            </span>
          </CardTitle>
          <p className="text-sm text-muted-foreground">
            Your group financial overview — {groupStats.totalMembers} members · {groupStats.interestRate}% monthly interest · Share value {formatCurrency(groupStats.shareValue)}
          </p>
        </CardHeader>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="bg-white dark:bg-gray-800 p-4 rounded-lg border shadow-sm">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Total Group Savings</span>
              </div>
              <div className="text-2xl font-bold text-green-600">{formatCurrency(groupStats.totalSavings)}</div>
              <div className="text-xs text-muted-foreground mt-1">From {groupStats.totalMembers} members</div>
            </div>

            <div className="bg-white dark:bg-gray-800 p-4 rounded-lg border shadow-sm">
              <div className="flex items-center gap-2 mb-1">
                <TrendingUp className="h-4 w-4 text-orange-600" />
                <span className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Total Loans Given</span>
              </div>
              <div className="text-2xl font-bold text-orange-600">{formatCurrency(groupStats.totalLoansOutstanding)}</div>
              <div className="text-xs text-muted-foreground mt-1">Outstanding to members</div>
            </div>

            <div className="bg-white dark:bg-gray-800 p-4 rounded-lg border shadow-sm">
              <div className="flex items-center gap-2 mb-1">
                <Wallet className="h-4 w-4 text-blue-600" />
                <span className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Cash in Box</span>
              </div>
              <div className="text-2xl font-bold text-blue-600">{formatCurrency(groupStats.totalCashInBox)}</div>
              <div className="text-xs text-muted-foreground mt-1">Available for lending</div>
            </div>
          </div>

          {/* Personal summary row */}
          <div className="border-t pt-4">
            <h4 className="text-sm font-semibold text-muted-foreground mb-3 uppercase tracking-wide">Your Personal Summary</h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="flex justify-between items-center bg-white dark:bg-gray-800 p-3 rounded-lg border">
                <span className="text-sm text-muted-foreground">My Savings</span>
                <span className="font-bold text-green-600">{formatCurrency(parseFloat(member.savingsBalance || '0'))}</span>
              </div>
              <div className="flex justify-between items-center bg-white dark:bg-gray-800 p-3 rounded-lg border">
                <span className="text-sm text-muted-foreground">My Share of Group</span>
                <span className="font-bold">
                  {groupStats.totalSavings > 0
                    ? ((parseFloat(member.savingsBalance || '0') / groupStats.totalSavings) * 100).toFixed(1)
                    : '0.0'}%
                </span>
              </div>
              <div className="flex justify-between items-center bg-white dark:bg-gray-800 p-3 rounded-lg border">
                <span className="text-sm text-muted-foreground">My Loan Balance</span>
                <span className="font-bold text-orange-600">{formatCurrency(parseFloat(member.currentLoan || '0'))}</span>
              </div>
              <div className="flex justify-between items-center bg-white dark:bg-gray-800 p-3 rounded-lg border">
                <span className="text-sm text-muted-foreground">Loan Due Date</span>
                {member.activeLoan?.dueDate ? (
                  <span className={`font-bold ${new Date(member.activeLoan.dueDate) < new Date() ? 'text-red-600' : 'text-primary'}`}>
                    {new Date(member.activeLoan.dueDate).toLocaleDateString('en-UG', { day: 'numeric', month: 'short', year: 'numeric' })}
                    {new Date(member.activeLoan.dueDate) < new Date() && (
                      <span className="ml-1 text-xs text-red-500">(Overdue)</span>
                    )}
                  </span>
                ) : (
                  <span className="text-sm text-muted-foreground">No active loan</span>
                )}
              </div>
            </div>
          </div>
      ...
      {/* Full Group Financial Summary - Committee members only */}
      {isLeadershipRole && (
        <Card className="border-2 border-primary/20 bg-gradient-to-r from-primary/5 to-blue-50 dark:from-primary/10 dark:to-blue-900/20">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-lg sm:text-xl">
              ...
              <span className="text-primary font-bold">Full Group Financial Summary</span>
            </CardTitle>
            <p className="text-sm text-muted-foreground">
              Complete overview — committee access only
            </p>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
              ...
              <div className="bg-white dark:bg-gray-800 p-4 rounded-lg border shadow-sm">
                ...
                <div className="text-xs text-muted-foreground mt-1">Available for loans</div>
              </div>

              <div className="bg-white dark:bg-gray-800 p-4 rounded-lg border shadow-sm">
                <div className="flex items-center gap-2 mb-2">
                  <TrendingUp className="h-4 w-4 text-orange-600" />
                  <span className="text-sm font-medium text-muted-foreground">Loans Outstanding</span>
                </div>
                <div className="text-xl sm:text-2xl font-bold text-orange-600">
                  {formatCurrency(groupStats.totalLoansOutstanding)}
                </div>
                <div className="text-xs text-muted-foreground mt-1">Outstanding amount</div>
              </div>
              ...
              <div className="bg-white dark:bg-gray-800 p-4 rounded-lg border shadow-sm">
                ...
                <div className="text-xs text-muted-foreground mt-1">At {groupStats.interestRate}% rate</div>
              </div>
            </div>
            ...
            <div className="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div className="bg-white dark:bg-gray-800 p-3 rounded-lg border">
                <div className="text-sm text-muted-foreground mb-1">Original Loan Principal</div>
            ...
            {/* Detailed breakdown */}
            <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 pt-4 border-t">
              <div>
              ...
              <div>
                ...
                <div className="space-y-1 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{formatCurrency(groupStats.totalCashInBox)}</span>
                  </div>
                </div>
              </div>
              <div className="sm:col-span-2 lg:col-span-1">
                ...
                <div className="space-y-1 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{formatCurrency(groupStats.totalOriginalLoans)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Total Interest:</span>
                    <span className="font-medium text-indigo-600">{formatCurrency(groupStats.totalInterest)}</span>
                  </div>
                  <div className="flex justify-between">
